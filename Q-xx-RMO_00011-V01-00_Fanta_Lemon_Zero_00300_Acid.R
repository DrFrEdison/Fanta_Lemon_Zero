...
dir( pattern = "Rsource" )
source.file <- print("Rsource_Fanta_Lemon_Zero_mtx_mop_val_V01.R")
...
lapply(dt$raw, nrow)
dt$raw <- lapply(dt$raw, \( x ) x[ seq(1, nrow(x), 8) , ])
...
fwrite(cbind( dt$model.raw$data, dt$model.raw$spc), paste0(datetime(), "_", dt$para$beverage, "_", dt$para$substance[dt$para$i], "_matrix.csv"), row.names = F, dec = ",", sep = ";")
...
dt$pls$merge <- lapply(dt$pls$pred, function( x ) merge_pls(pls_pred = x, pls_lm = dt$pls$lm, mean = c(100 * c(.8, 1.2)), R2=.8))
...
#   head(dt$pls$mergesite)} else{ 
    dt$pls$mergesite <- dt$pls$merge[[2]]
...
dt$pls$vas <- linearitaet_filter( linearitaet_prediction =  dt$pls$pred.vas$prediction
                                  , ncomp = dt$para.pls$ncomp
                                  , linearitaet_limit_1 = dt$vas$diff * .80
                                  , linearitaet_limit_2 = dt$vas$diff * 1.2
                                  , R_2 = .8
...
for(i in 1:length(dt$mop$pred)){
  ...
  abline( h = unlist(dt$para$eingriff[[ dt$para$i ]]) / dt$para$SOLL[ dt$para$i ] * 100, col = "orange", lty = 2 )
  abline( h = unlist(dt$para$sperr[[ dt$para$i ]]), col = "red", lty = 2 )
}
lapply(dt$mop$pred, function( x ) sd( x,na.rm = T))
lapply(dt$mop$pred, function( x ) mad( x,na.rm = T))
