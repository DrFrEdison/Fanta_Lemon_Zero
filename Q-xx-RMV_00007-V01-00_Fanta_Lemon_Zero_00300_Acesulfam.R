...
dt$model$file <- grep(dt$para$beverage, dir( paste0(wd$model, "/", dt$para$customer) ), value = T)
dt$model$file <- grep(dt$para$substance, dt$model$file, value = T)[ 1 ]

file.copy( paste0(wd$model[[ 1 ]], "/", dt$para$customer, "/", dt$model$file) 
           , dt$model$file, overwrite = T)
...
dt$para$model.name <- grep(".unsb", grep( "TTA" , dir(), value = T), value = T)
dt$para$model.name <- print(gsub(".unsb", "", dt$para$model.name))

# Validation table copy ####
for(i in 1:length(dt$para$trs)){
setwd(dt$wd)
setwd("./Modellvalidierung")
setwd(paste0("./", dt$para$val.date, "_", dt$para$model.pl[1], "_", dt$para$substance[dt$para$i]))
...
file.copy("D:/OneDrive - Dausch Technologies GmbH/Dokumentation/QM/06_FO_Formblaetter/FO-223-V01-0_Validierungstabelle_Mastermodell.xlsx"
          , dt$xlsx$file <- paste0(date(), "_Valdierung_"
                                   , dt$para$customer, "_", dt$para$beverage, "_"
                                   , dt$para$location[ i ], "_", dt$para$line[ i ], "_"
                                   , dt$para$substance[ dt$para$i ], "_V01_00.xlsx")
          , overwrite = F)

# Validierungsinfo ####
dt$xlsx$info <- read.xlsx(dt$xlsx$file, sheet = "Validierungsinfo", colNames = F)
dt$xlsx$info$X2
dt$xlsx$info$X2 <- c(dt$para$location[ i ]
                     , dt$para$line[ i ]
                     , dt$para$beverage
                     , dt$para$beverage
