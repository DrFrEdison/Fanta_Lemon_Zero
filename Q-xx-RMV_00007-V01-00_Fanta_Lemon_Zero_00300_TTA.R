...
dir( pattern = "_val_" )
source.file <- "Rsource_Fanta_Lemon_Zero_mtx_mop_val_V01.R"
...
keep.out.unsb.model(customer = dt$para$customer
                    ...
                    , LG = dt_customer$LG[which(dt_customer$line == as.character(dt$para$line[ 7 ]))]
...
plot( dt$pred$Moenchengladbach_G9 , axes = F, xlab = "", ylab = "TTA in %", main = dt$para$txt$loc.line[ 2 ])
xaxisdate( dt$trs$Moenchengladbach_G9$data$datetime )
...
# Protokoll
# Vergleichmessung Master Methode
# Vergleichmessung MMI Änderung
# Linearität

dt$lab <- lapply( grep( "KA-TSW-QS-FB42", dir( pattern = ".xlsx"), value = T), function( x ) read.xlsx( x, sheet = "Vergleichmessung Master Methode") )

dt$lab <- lapply( dt$lab, function( x ) x[ !is.na( x$Produkt) , ] )

for(i in 1:length(dt$lab)){ 
  dt$lab[[ i ]][ , grep( "Probenahme.Datum", colnames( dt$lab[[ i ]] ))] <- convertToDate( dt$lab[[ i ]][ , grep( "Probenahme.Datum", colnames( dt$lab[[ i ]] ))] )
  dt$lab[[ i ]][ , grep( "Probenahme.Uhrzeit", colnames( dt$lab[[ i ]] ))] <- strftime( convertToDateTime( dt$lab[[ i ]][ , grep( "Probenahme.Uhrzeit", colnames( dt$lab[[ i ]] ))] ), format = "%H:%M:%S")
  
  dt$lab[[ i ]][ , grep( "Analysedatum", colnames( dt$lab[[ i ]] ))] <- convertToDate( dt$lab[[ i ]][ , grep( "Analysedatum", colnames( dt$lab[[ i ]] ))] )
  dt$lab[[ i ]][ , grep( "Analysezeit.Labor", colnames( dt$lab[[ i ]] ))] <- strftime( convertToDateTime( dt$lab[[ i ]][ , grep( "Analysezeit.Labor", colnames( dt$lab[[ i ]] ))] ), format = "%H:%M:%S")
  }
